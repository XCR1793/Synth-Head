...
void drive_HUB75(int Fnum, int Buffcount, int Rvalues, int Gvalues, int Bvalues) {
...
for(int i = 1; i <= Buffcount; i++){
    ...
    for(int ii = 1; ii <= LED_Matrix_Length; ii++) {
        ...
        if((1 <= ((frame[Fnum][cycle ] & GBitMask)>>(Bvalues)          )-GColourSub*CurrentBuff)){digitalWrite(HUBG0,HIGH);/*Serial.print(" G0");*/} else {digitalWrite(HUBG0,LOW);}
        if((1 <= ((frame[Fnum][cycle ] & BBitMask)                     )-BColourSub*CurrentBuff)){digitalWrite(HUBB0,HIGH);/*Serial.print(" B0");*/} else {digitalWrite(HUBB0,LOW);}
        if((1 <= ((frame[Fnum][cycle2] & RBitMask)>>(Gvalues + Bvalues))-RColourSub*CurrentBuff)){digitalWrite(HUBR1,HIGH);/*Serial.print(" R1");*/} else {digitalWrite(HUBR1,LOW);}
        if((1 <= ((frame[Fnum][cycle2] & GBitMask)>>(Bvalues)          )-GColourSub*CurrentBuff)){digitalWrite(HUBG1,HIGH);/*Serial.print(" G1");*/} else {digitalWrite(HUBG1,LOW);}
        if((1 <= ((frame[Fnum][cycle2] & BBitMask)                     )-BColourSub*CurrentBuff)){digitalWrite(HUBB1,HIGH);/*Serial.print(" B1");*/} else {digitalWrite(HUBB1,LOW);}

        // Serial.print((frame[Fnum][cycle2]));
        // Serial.print(" ");
